...
void worker(int start, int num, long * cur, FILE * fso, int pipefd[]){
	/*
	...
	if(pid == 0){
		worker(start + 50, num-1, cur, fso, pipefd2);
		return;
	}else if(pid == -1)
		handle_error("fork");
	...
	if(pid != -2){ // checking if child was spawned, and now to deal with it
		...
		i = read(pipefd2[0], &buf, sizeof(long)); // j is an error catch - read will first read the sum found
		if(i == -1)
			handle_error("read01");
		sum += buf;

		i = read(pipefd2[0], &buf, sizeof(long)); // then it reads the min found
		if(i == -1)
			handle_error("read02");
		if(buf < min)
			min = buf;

		i = read(pipefd2[0], &buf, sizeof(long)); // and finally the max that was found is read
		if(i == -1)
			handle_error("read03");
		if(max < buf)
			max = buf;
...
void launch(int argc, char ** argv){
	...
	char * filename = (char *)malloc((strlen(argv[1])+1) + 12); // length of original filename + 1 for null byte + 12 to be able to add '_outputB.txt' at end
	...
	filename = strcat(filename, "_outputB.txt"); // add _output.txt to end
	...
	int extra = i % 50; // number of extra inputs that need to processed by this first process
	...
	if(num > 1){ // only fork if there is more than 9 numbers
		if(pipe(pipefd) == -1)
			handle_error("pipe");
		pid = fork();
	}
	...
	if(pid == 0){
		worker(extra, num -2, cur, fso, pipefd); //num -2 since there is a parent already and the child has been made
		exit(EXIT_SUCCESS);
	}else if(pid == -1)
		handle_error("fork");

	/*
	 * At this point the main process has forked off a child, which will start doing the things in worker.
	...
	// local values found, now have to receive child's numbers

	if(pid != -2){
		close(pipefd[1]); // close writing portion of pipe
		long buf; // buffer to hold incoming longs

		j = read(pipefd[0], &buf, sizeof(long)); // j is an error catch - read will first read the sum found
		if(j == -1)
			handle_error("read04");
		sum += buf;

		j = read(pipefd[0], &buf, sizeof(long)); // then it reads the min found
		if(j == -1)
			handle_error("read05");
		if(buf < min)
			min = buf;

		j = read(pipefd[0], &buf, sizeof(long)); // and finally the max that was found is read
		if(j == -1)
			handle_error("read06");
		if(max < buf)
			max = buf;
